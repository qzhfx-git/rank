...
score_labels = {'A':5,'B':5,'C':10,'D':10,'E':10,'F':20,'G':20,'H':20,'I':30,'J':30,'K':30}
# URL for contest data
DATA_URL = 'http://172.23.71.17/contestrank-oi.php?cid=1002'
            ...
            for row in rows[1:]:  # 跳过表头
                ...
                try:
                    ...
                    for i, label in enumerate(problem_labels):
                        ...
                        scores[label] = int(score/100 * score_labels[label])
                    ...
                    if participant['id'].startswith('T'):
                        participant['id'] = participant['id'].lstrip('T')
                        pro_group.append(participant)
                    else:
                        participant['id'] = participant['id'].lstrip('Y')
